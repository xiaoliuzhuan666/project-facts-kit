...
4. Run `ai-context-kit inspect --workspace <path>` when the workspace's existing standards or acceptable write scope are not yet known. `inspect` writes nothing, skips implementation-source reads, and reports detected repositories, `AGENTS.md`, project-facts, OpenSpec, Spec Kit, MADR, Backstage, Continue rules and CODEOWNERS paths. Run `ai-context-kit onboard --workspace <path>` only after generated workflow files are acceptable; it generates missing artifacts, then prints `doctor`, `token-status`, and `capability actions`. If `ai-context-kit` is not on `PATH` but the kit is cloned under `~/.cache/project-facts-kit`, run `~/.cache/project-facts-kit/scripts/setup-local-kit.sh` when local setup is requested, or use `node ~/.cache/project-facts-kit/packages/ai-context-kit/bin/ai-context-kit.mjs <command> --workspace <path>` and record the fallback. If the kit is not cloned, tell the user to clone or update the internal repository before using project entry phrases.
...
1. Start at the parent workspace with `ai-context-kit inspect --workspace <path>` when existing standards or the write boundary are unknown. Run `onboard` before broad source search only when writing generated files is acceptable; otherwise continue with `doctor` and targeted reads.
