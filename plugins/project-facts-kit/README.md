...
Codex marketplace plugin package for sharing the project facts and low-token
context skills.
...
The marketplace entry in `.agents/plugins/marketplace.json` points to this
directory as a local plugin source, so the plugin can be distributed through a
Codex repo or team marketplace.

If the plugin is installed from a public GitHub repository, first clone or update the
kit and run local setup:
