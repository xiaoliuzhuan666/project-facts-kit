...
6. Candidate files and daily summaries must use repository-relative paths, symbols, commands, validation status and sanitized summaries only. Do not copy business data, user data, request or response bodies, raw logs, database rows, Codex event payloads, tokens, cookies, API keys, passwords, private keys or production environment values.
7. Run `ai-context-kit redact --input <raw-file> --output <redacted-file>` before summarizing logs, API responses, stderr, Codex events or task traces. If safety is still unclear, cite only the local evidence path and a short sanitized summary.
8. Use `ai-context-kit automation-prompt --workspace <workspace> --type skill-feedback-candidate` when a Codex app automation prompt is needed. This command prints prompt text only; it does not create the automation.
