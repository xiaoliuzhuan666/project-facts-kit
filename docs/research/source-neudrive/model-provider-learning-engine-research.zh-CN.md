# xlzDrive 可配置模型提供方 + 每日学习任务 + 成长提案系统调研
...
Aider 的 repo map 思路很适合 xlzDrive。它不是把整个仓库塞给模型，而是先压缩成结构化索引。
...
这是和很多“自进化”工具拉开距离的地方。xlzDrive 应该强调：
...
xlzDrive 已经有 Claude Code / Codex 同步和 Cursor / Gemini CLI 导出边界。成长系统要服务这条链路：
...
```json
{
  "version": "xlzdrive.model-providers/v1",
...
```json
{
  ...
  "source_paths": [
    ...
    "/skills/docker-delivery/manifest.neudrive.json"
  ],
  "created_by": {
