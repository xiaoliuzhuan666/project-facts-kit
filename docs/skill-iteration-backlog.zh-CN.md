...
| `SFC-20260829-fragment-latest-adoption-tracking` | `docs/skill-feedback/2026-08-29-fragment-latest-adoption-tracking.md` | `tooling_fix` | `proposed` | 2026-08-29 登记：upgrade 写入的 `AGENTS.fragment.latest.md` 无落地跟踪，崆峒悬置 28 天未被合并（同期反哺候选却活跃）；候选方案为 upgrade 追加待办/提醒、doctor 报 `fragment pending adoption`、adoption-guide 增加收尾步骤；Reviewer decision 为 `Pending`，保持 `proposed`。 |
...
| 日期 | 候选项 | 结果 | 评审摘要 | 下一步 |
| --- | --- | --- | --- | --- |
| 2026-07-26 | `SFC-20260715-fast-local-fix-context-budget` | `applied` | 分支 `codex/low-token-fast-local-path` 已通过 [PR #4](https://github.com/xiaoliuzhuan666/project-facts-kit/pull/4)（merge commit `79e231f`，2026-07-21）合入 `main`；`skills/low-token-context-maintainer/SKILL.md` 已含 Fast Local Fix Path 小节，候选文件状态同步翻为 `applied`。 | 真实项目接入、升级、token 测量和 hosted GitHub Actions 仍为 `Not run`。 |
