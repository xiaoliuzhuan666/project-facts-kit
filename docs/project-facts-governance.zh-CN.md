...
- 工具和流程的复杂度归维护者，普通使用者只记三句入口；任何新能力不得增加使用者必学的命令或概念。
...
反哺改动按影响范围分三层（2026-07-21 起）：

1. 项目本地层：目标项目自己的 `project-facts/`、`AGENTS.md` 和项目级规则，立即生效，不需要本资料库评审。
2. 共享低风险层：文档措辞、提示词文案和新增 reference 小节，资料库检查通过并开放 7 天异议窗口后可合并；owner 显式批准可立即合并。
3. 共享高风险层：Skill 工作流规则、模板结构、CLI 行为和本制度正文，必须经 Tool/library owner 显式批准。
